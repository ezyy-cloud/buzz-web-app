    ...
    <v-container>
        <form @submit.prevent="submit">
            ...
            <v-btn class="me-4 float-right" type="submit">
                        ...
                        <v-container>
                            <v-form @submit.prevent>
                                <v-text-field v-model="confirmationPairCode" :rules="rules"
                                    :label="`Enter the pair code to confirm deletion: ${props.node.pairCode}`"></v-text-field>
                                <v-btn block :disabled="loading" v-if="confirmationPairCode == props.node.pairCode"
                                    class="mt-2 text-red" @click="">Delete Node</v-btn>
                            </v-form>
                        </v-container>
                        <v-card-actions>
                ...
                <v-btn class="text-red font-weight-black" variant="text" @click="dialog = true">DELETE THIS NODE</v-btn>
