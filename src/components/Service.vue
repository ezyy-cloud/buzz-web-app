<template>
  <div id="scroll-threshold-example">
    <v-container class="pa-0 d-flex align-start pb-8">
        ...
        <v-row align="start" justify="space-between">
          ...
          <v-container class="col-lg-6 col-md-12 col-sm-12 col-sm-12">
            ...
            >
              ...
              >
                ${{ service($route.params.id).price }}
              </div>
              <div
                class="text-lg-h5 text-md-h6 text-sm-h6 text-h6 font-weight-thin"
              >
                {{ service($route.params.id).reviews.length }} reviews
              </div>
            </v-row>
            <v-divider></v-divider>
            <v-row
      ...
      >
        ...
        <div
          ...
          v-for="item in cart('service')"
          :key="item.id"
        >
          <div>{{ service(item.id).title }}</div>
          <div>{{ item.quantity }}</div>
          <div>${{ service(item.id).price * item.quantity }}</div>
        </div>
        <div class="overline">
          Total:
        </div>
        <div class="text-h4 font-weight-light ma-4">
          ${{ cartTotal(cart("service")) }}
        </div>
    ...
    >
      <v-btn disabled text>${{ cartTotal(cart("service")) }}</v-btn>
...
export default Vue.extend({
  ...
  methods: {
    cartTotal(itemArray: any[]) {
      let sum = 0;
      itemArray.forEach(
        item => (sum += this.service(item.id).price * item.quantity)
      );
      return sum;
    },
    ...mapActions({
  ...
  computed: {
    ...
    ...mapGetters({
      ...
      rating: "services/getRating",
      about: "services/getAbout",
      requirements: "services/getRequirements",
      specifications: "services/getSpecifications",
      tAndCs: "services/getTandCs",
      images: "services/getImages",
      cart: "carts/getCart",
