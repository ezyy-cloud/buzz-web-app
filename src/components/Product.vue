  ...
  <div id="scroll-threshold-example">
    <v-container class="pa-0 d-flex align-start pb-8">
      <v-container class="col-lg-9 col-md-10 col-sm-10 col-11 pa-0">
        <v-row>
          ...
          <v-container class="col-lg-6 col-md-12 col-sm-12 col-sm-12">
            ...
            >
              ...
              >
                ${{ product($route.params.id).price }}
              </div>
              <div
                class="text-lg-h5 text-md-h6 text-sm-h6 text-h6 font-weight-thin"
              >
                {{ product($route.params.id).reviews.length }} reviews
              </div>
            </v-row>
            <v-divider></v-divider>
            <v-row
        ...
        <v-container class="d-flex justify-center flex-column col-8 mt-4 mb-4">
      ...
      >
        ...
        <div
          ...
          :key="item.id"
        >
          <div>{{ product(item.id).title }}</div>
          <div>{{ item.quantity }}</div>
          <div>${{ product(item.id).price * item.quantity }}</div>
        </div>
        <div class="overline">
          Total:
        </div>
        <div class="text-h4 font-weight-light ma-4">
          ${{ cartTotal(cart("product")) }}
        </div>
    ...
    >
      <v-btn disabled text>${{ cartTotal(cart("product")) }}</v-btn>
...
export default Vue.extend({
  ...
  methods: {
    cartTotal(itemArray: any[]) {
      let sum = 0;
      itemArray.forEach(
        item => (sum += this.product(item.id).price * item.quantity)
      );
      return sum;
    },
    ...mapActions({
  ...
  computed: {
    ...
    ...mapGetters({
      ...
      about: "products/getAbout",
      requirements: "products/getRequirements",
      specifications: "products/getSpecifications",
      tAndCs: "products/getTandCs",
